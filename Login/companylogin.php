...
<!-- NAVIGATION STARTS-->
    ...
    if(isset($_POST['submit'])){
        ...
        $email_search = "select * from registeredcompany where comusername='$username'";
        ...
        if($email_count)
        {
            $sql = "Select compassword, compname from registeredcompany where comusername='$username'";
            $hash= mysqli_query($conn, $sql);
            while($row = mysqli_fetch_assoc($hash)){
            $hash = $row["compassword"];
...
?>
    ...
    <h3 style="text-align: center;">Job Recruiter Login Panel</h3>
    <br><br><br>
