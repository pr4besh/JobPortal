...
<!-- NAVIGATION STARTS-->
    ...
    <div class="elearning" style="padding-top: 50px;">
        ...
        <?php 
            ...
            $eduId = $_GET["eduId"];
            $sql=mysqli_query($conn, "select * from education where eduId = $eduId");
            while($row=mysqli_fetch_assoc($sql)){
        ?>
            <!-- FORM STARTS -->
            <br>
            <form action="educationUpdate.php" class="reporting form" METHOD="POST">
            <section style="display: flex;">
            <div class="left" style="padding-left: 150px;">
                <div class="input-group">
                    <label for="course"><h6>Course</h6></label>
                    <input name="course" value="<?=$row["course"]?>" type="text" placeholder="<?=$row["course"]?>" id="course"><br>
                    <span id="course"> </span>
                </div>
                <div class="input-group">
                    <label for="university"><h6>University</h6></label>
                    <input name="university" type="text" value="<?=$row["university"]?>" id="university"><br>
                    <span id="university"> </span>
                </div>
                <div class="input-group">
                    <label for="passingYear"><h6>Passing Year</h6></label>
                    <input name="passingYear" type="text" value="<?=$row["passingYear"]?>" id="passingYear"><br>
            ...
            <div class="input-group">
                    ...
                    <input name="specialization" type="text" value="<?=$row["specialization"]?>" id="specialization"><br>
                    <span id="specialization"> </span>
                </div>
                <div class="input-group">
                    <label for="institution"><h6>Institution</h6></label>
                    <input name="institution" type="text" value="<?=$row["institution"]?>" id="institution"><br>
                    <span id="institution"></span>
                </div>
                
                <div class="input-group">
                    <label for="educationId"><h6>Education Id</h6></label>
                    <input name="educationId" type="text" value="<?=$row["eduId"]?>" readonly="educationId" id="educationId"><br>
                    <span id="educationId"> </span>
                </div>
            </div>
            </section>
            <?php } ?>
