...
<!-- NAVIGATION STARTS-->
    ...
    if(isset($_POST['submit'])){
        ...
        $email_search = "select * from customer where cususername='$username'";
        ...
        if($email_count)
        {
            $sql = "Select cuspassword,name,c.regid from customer c join registrationdetails r where c.regid = r.regid and c.cususername='$username'";
            $hash= mysqli_query($conn, $sql);
            while($row = mysqli_fetch_assoc($hash)){
            $hash = $row["cuspassword"];
